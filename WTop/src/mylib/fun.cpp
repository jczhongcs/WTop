...
long mylib::fun::Ali2_To_AfpCor(long n1, long n2, vector<long> &ali2,
        ...
        if (ali2[i] == -1) //purely blank
        {
            if (isFirst == 0) {
                if (count >= thres) {
                    ...
                    //----- check cross_over -----//
                    if (afp_num > 1) {
                        if (AFP_Cor[afp_num - 2][0] + AFP_Cor[afp_num - 2][2] > AFP_Cor[afp_num - 1][0] ||
...
long mylib::fun::Ali1_To_AfpCor(long n1, long n2, vector<long> &ali1,
                                vector<vector<long> > &AFP_Cor, long thres) {
    //from ali2 to ali1
...
void mylib::fun::AfpCor_To_Ali1_Ali2(long n1, long n2,
...
void mylib::fun::Ali1_To_AliPair(long n1, long n2, vector<long> &ali1,
...
void mylib::fun::Ali2_To_AliPair(long n1, long n2, vector<long> &ali2,
                                 vector<pair<long, long> > &alignment_out) {
    //from ali1 to ali2
...
void mylib::fun::AliPair_To_Ali1_Ali2(long n1, long n2,
...
void mylib::fun::From_Align_Get_Bound(long moln1, long moln2, vector<pair<long, long> > &align,
...
void mylib::fun::Renmin_To_Sheng_align(long moln1, long moln2,
...
//         we use 'first-come never-gone' strategy
//[note2]: moln1 MUST less than moln2
void mylib::fun::Sheng_To_Renmin_align(long moln1, long moln2,
                                       vector<pair<long, long> > &align_in, vector<pair<long, long> > &align_out) {
    vector<long> ali1(moln1, -1);
    vector<long> ali2(moln2, -1);
...
void mylib::fun::Sheng_To_Renmin_bound(long moln1, long moln2,
...
void mylib::fun::ZScoreNormalize(std::vector<double> &signals, double *avg, double *stdev) {
    if (signals.empty()) {
        if (avg) *avg = 0.0;
        if (stdev) *stdev = 0.0;
        return;
    ...
    // 防止除以0：若deviation为0，说明全体值一致（或全0），标准化后统一设为0
    if (deviation > 1e-12) {
        for (size_t i = 0; i < signals.size(); ++i) {
            signals[i] /= deviation;
        }
...
void mylib::fun::Diff(const std::vector<double> &raw, std::vector<double> &diff) {
...
void mylib::fun::PeakPick(const std::vector<double> &raw, std::vector<std::pair<long, double> > &peaks) {
    peaks.clear();
    ...
    for (long i = 0; i < diff.size() - 1; i++) {
        if ((diff[i] > EQN_EPS && diff[i + 1] < -1 * EQN_EPS) ||
            (diff[i] < -1 * EQN_EPS && diff[i + 1] > EQN_EPS)) {        //local max min
...
double mylib::fun::DynamicTimeWarping(
...
double mylib::fun::DynamicTimeWarping_global(
        const std::vector<double> &seq1, const std::vector<double> &seq2,
        std::vector<std::pair<long, long> > &alignment) {
    //-- create score matrix --//
...
double mylib::fun::DynamicTimeWarping_local(
        const std::vector<double> &seq1, const std::vector<double> &seq2,
        std::vector<std::pair<long, long> > &alignment) {
    //-- create score matrix --//
    double *score[seq1.size()];
...
static inline double &SCORE(long i, long j,
                            ...
                            const std::vector<std::pair<long, long> > &bound) {
...
double mylib::fun::BoundDynamicTimeWarping(
    ...
    for (long i = 1; i < seq1->size(); i++) {
        for (long j = bound[i].first; j <= bound[i].second; j++) {
            double acc = std::min(std::min(SCORE(i - 1, j, score, bound), SCORE(i, j - 1, score, bound)),
                                  SCORE(i - 1, j - 1, score, bound));
            if (acc == DBL_MAX) {
...
void mylib::fun::combine(const vector<double> &ref, const vector<double> &peer, vector<double> &signal) {
    int i = 0;
    for (i = 0; i < ref.size(); i++) {
        signal.push_back(ref[i]);
    }
    for (i = 0; i < peer.size(); i++) {
        signal.push_back(peer[i]);
    }
    double avg, stdev;
    mylib::fun::ZScoreNormalize(signal, &avg, &stdev);
}

void mylib::fun::splite(const vector<double> &ref, const vector<double> &peer, vector<double> &signal,
                        vector<double> &ref_p, vector<double> &peer_p) {
    int i = 0;
    for (i = 0; i < ref.size(); i++) {
        ref_p.push_back(signal[i]);
    }
    for (; i < signal.size(); i++) {
        peer_p.push_back(signal[i]);
    }
}
